...
| プロセスノード、メモリ製品、工場投資、買収、企業の資本・人事、事故・災害 | `si` | si-9, si-83, si-95 |
| AI演算チップ(GPU・TPU・NPU・アクセラレータ)の発表と量産、AIモデル、計算契約、データセンター投資、AIラボの資金調達 | `ai` | ai-16, ai-61, ai-123 |
...
- **AI演算チップの発表は `ai`、メモリとプロセスは `si`。** GPU・TPU・NPU・アクセラレータとその供給契約は、半導体製品ではあるが `ai` に置く。NVIDIA Blackwell だけ `si` で Blackwell Ultra と Vera Rubin が `ai`、AMD の MI300X が `si` で AMD–OpenAI の GPU 契約が `ai`、Tesla の FSD チップが `ai` で Dojo と AI5 が `si`、という状態を作らない。HBM・DRAM・NAND とプロセスノードは `si`。
- **同じ製品系列を2つの系列に分けない。** 迷ったら「この出来事は、既にある兄弟の出来事とどこにあるか」を先に調べる。
