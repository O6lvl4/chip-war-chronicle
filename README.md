...
| 機能 | 説明 |
|---|---|
| 番組表レイアウト | 6系列を縦に並べ、横軸はひとつの時間。各系列の中では出来事を重ならない行に詰め、行数に応じて系列の高さが伸びる。盤面は縦にスクロール、系列はヘッダーのチップで個別にオン・オフ |
| セマンティックズーム | ⌘/Ctrl+ホイール、ピンチ、＋−ボタン、ダブルクリックで拡大縮小(操作中は合成レイヤーを伸縮させ、指を離した時点で1回だけ再配置)。横ホイール・横スワイプ・ドラッグはいまの縮尺のまま横移動、縦ホイール・縦スワイプは盤面のスクロール。目盛が 年 → 四半期 → 月 → 週 に切り替わり、広い表示ではラベルを重要度で間引く |
