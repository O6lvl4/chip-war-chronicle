...
| 機能 | 説明 |
|---|---|
| 番組表レイアウト | 6系列を縦に並べ、横軸はひとつの時間。各系列の中では出来事を重ならない行に詰め、行数に応じて系列の高さが伸びる。時間軸は上に、系列名は左に貼り付き(sticky)、盤面は縦横ともブラウザ標準のスクロール。系列はヘッダーのチップで個別にオン・オフ |
| 離散ズーム | 番組表の 30分/1時間 の刻みと同じで、倍率は 12 段(0.25〜40 px/日)の固定レベル。⌘/Ctrl+ホイール、ピンチ、ダブルクリック、`+` `-` で切り替え。操作中は描画済みタイルを CSS で伸縮させ、指を離した時点で最寄りのレベルに吸着して描き直す(旧レベルの絵は新しいタイルが揃うまで残る)。目盛が 年 → 四半期 → 月 → 週 → 日 に切り替わり、粗いレベルではラベルを重要度で間引く |
| 因果リンク | 出来事をクリックすると、引き金になった出来事と引き起こした出来事がタイルの上のレイヤーに線とリングで描かれる。全部の線は描かない |
| 共有 URL | 表示レベル・中心日・選択・系列・テーマを URL ハッシュに持つ。リンクを開けば同じ画面になる |
...
- 盤面は「地図アプリ」の作り。倍率は離散レベルで、レベルごとに行詰め(どの出来事が何行目か、ラベルを出すか)を Web Worker で1回だけ計算する。描画は 512px × 系列 のタイル単位で、Worker の OffscreenCanvas が描いた ImageBitmap をメインスレッドが受け取るだけ。見えている範囲 ± 1 画面分だけ `<canvas>` を置き、スクロールはネイティブ(慣性・軸ロック込み)。メインスレッドはスクロール中に絵を描かない。因果リンクと検索のハイライトはタイルの上の SVG 1枚。`?perf=1` で端末上のフレーム/タイル統計を表示
- 性能予算: CI で Playwright(Pixel 5 相当)が盤面をフリックし、34ms 超のフレーム数とタイル描画 p95 が閾値を超えるとデプロイしない(`perf/fling.mjs`)
